...
class CorporateReportBuilder:
    ...
    def browse_file(self):
        default_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Raw files")
    ...
    def preview_chart(self):
        ...
        try:
            ...
            if chart_type == "Bar":
                ...
                fig = go.Figure(data=[go.Pie(labels=labels[:pie_n], values=values[:pie_n])])

            fig.update_layout(title=f"{chart_type} Chart: {data.columns[1]} by {data.columns[0]}")

            # Save to temp PNG
            temp_fd, temp_path = tempfile.mkstemp(suffix='.png')
            os.close(temp_fd)
            fig.write_image(temp_path, width=520, height=420)
            
            self.current_figure = fig  # Store plotly figure for export

            # Display in Tkinter
            image = Image.open(temp_path)
            photo = ImageTk.PhotoImage(image)
            
            label = tk.Label(self.chart_container, image=photo)
    ...
    def export_chart(self):
        ...
        try:
            self.current_figure.write_image(out_path, width=800, height=600)
